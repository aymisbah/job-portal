...
const SearchInput=({placeholder , icon , value, setValue, styles}) => {
    const handleChange = () =>{ setValue(e.target.value)}
...
const Header = ({title, type, handleClick, searchQuery, setSearchQuery,location,setLocation}) => {
  return (
    <div className='bg-[#fff]'>
      <div className={`container mx-auto px-5 ${type ? "h-[500px]" : "h-[350pc]"}flex items-center relative`}>
        <div className='w-full z-10'>
            ...
            <div className='w-full flex items-center justify-around bg-white pz-2 md:px-5 py-2.5 md:py-6 shadow-2xl rounded-full'>
                <SearchInput placeholder='Job Title or Keywords' icon={<AiOutlineSearch/>} className='text-gray-600 text-xl'
                value={searchQuery} setValue={setSearchQuery}/>
                <CustomButton onClick={handleClick} title="Search" containerStyles={"text-white py-2 md:3 px-3 md:px-10 focus:outline-none bg-blue-900 rounded-full md:rounded-md text-sm md:text-base"}/>
            </div>
            <div>
            {type && (
